...
### Per-channel streaming overrides

You can override the account-level streaming settings for individual channels. This is useful when streaming causes issues in specific channels (e.g. as a workaround for bug #20273) while keeping it enabled elsewhere.

```json5
{
  channels: {
    slack: {
      streaming: "partial", // default for all channels
      channels: {
        "#important-channel": {
          allow: true,
          streaming: "off", // disable streaming for this channel only
        },
        "#demo-channel": {
          allow: true,
          nativeStreaming: false, // keep draft preview but disable native streaming
        },
      },
    },
  },
...
When a channel entry has no `streaming` or `nativeStreaming` field, the account-level values are used. DMs always use account-level settings.
